DefineConstant[
  gap_Core = {0.0, Min 0, Max 1e-3, Step 1e-3,
    Name "Air gap"}
  central_width_Core_Leg = {0.11,
    Name "Core Geometry/width central leg"}
  width_Core_Leg = {0.16,
    Name "Core Geometry/width middle leg"}  
  width_Window = {0.35,
...
mm = 1e-3;
// Dimensions
external_width_Core_Leg = width_Core_Leg*2/3;
If (Config_prim == 1)
  If (Config_second == 1)
...
ElseIf (Config_prim == 2)
  ...
  EndIf
EndIf
width_Coil_PRIM = 4.2*mm;
width_Coil_SECOND = 10.2*mm;
...
c_Coil_1 = height_Coil_PRIM/2/10 *s;
c_Coil_2 = height_Coil_PRIM/2/10 *s;
c_Coil_3 = height_Coil_PRIM/2/10 *s;

c_Box = gap_Core_Box_X/4. *s;
