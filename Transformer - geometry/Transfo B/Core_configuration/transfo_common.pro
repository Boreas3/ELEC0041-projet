DefineConstant[
    ...
    n_PRIM = {200, Min 100, Max 1000, Step 100, Name "Number of turn - primary side"}
...
If (Config_prim == 1)
  If (Config_second == 1)
    n_SECOND = Ceil[n_PRIM/V_ratio*1.01];
  ElseIf (Config_second == 2)
    n_SECOND = Ceil[Sqrt[3] * n_PRIM/V_ratio*1.01];
  EndIf
ElseIf (Config_prim == 2)
  If (Config_second == 1)
    n_SECOND = Ceil[n_PRIM/(V_ratio*Sqrt[3])*1.01];
  ElseIf (Config_second == 2)
    n_SECOND = Ceil[n_PRIM/V_ratio*1.01];
  EndIf
EndIf
width_Coil_PRIM = 2*5.53*mm;
width_Coil_SECOND = 2*21.02*mm;
height_Coil_PRIM = n_PRIM*width_Coil_PRIM;
height_Coil_SECOND = n_SECOND*width_Coil_SECOND;
width_Core = width_Window+2*width_Core_Leg;
...
height_Core = height_Window+2*width_Core_Leg;
...
gap_Core_Box = 1.;
gap_Transfo = height_Core/2;
coef = height_Core + 4*gap_Transfo;
