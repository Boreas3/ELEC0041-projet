...
class XLSX_WRITE:
    def __init__(self, l, width_center, width, t):
        self.writer = pd.ExcelWriter("Magnetodynamics\\Three_Phases\\CoreA\\geotypeA2_{}.xlsx".format(l), engine='xlsxwriter')
        ...
        for i in range(0,N):
            for j in range(0,N):
                for k in range(0,N_t):
                    call(["gmsh", "-2", "Magnetodynamics/Three_Phases/CoreA/transfo_3p_core2.geo",  "-v", "1", 
                    ...
                    call(["getdp", "Magnetodynamics/Three_Phases/CoreA/transfo_3p_core2.pro", "-v", "1", "-solve", "Magnetodynamics2D_av", "-pos", "Map_a", 
                                                                            "-setnumber", "width_Window", "{}".format(width_window[l]),
                                                                            "-setnumber", "central_width_Core_Leg", "{}".format(width_center[i]),
                                                                            "-setnumber", "width_Core_Leg", "{}".format(width[j]),
                                                                            "-setnumber", "thickness_Core", "{}".format(t[k])])
                    temp_res = np.zeros([2,1])
                    with open('Magnetodynamics\\Three_Phases\\CoreA\\UI3_core2.txt', newline='') as csvfile:
