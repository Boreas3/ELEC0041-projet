...
class XLSX_WRITE:
    def __init__(self, width_center, width, t):
        self.writer = pd.ExcelWriter("Magnetodynamics\\Three_Phases\\ShellB\\geotypeB.xlsx", engine='xlsxwriter')
    ...
    for i in range(0,N):
        for j in range(0,N):
            for k in range(0,N_t):
                call(["gmsh", "-2", "Magnetodynamics/Three_Phases/ShellB/transfo_3p_shell.geo",  "-v", "1", 
                ...
                call(["getdp", "Magnetodynamics/Three_Phases/ShellB/transfo_3p_shell.pro", "-v", "1", "-solve", "Magnetodynamics2D_av", "-pos", "Map_a", 
                                                                        "-setnumber", "central_width_Core_Leg", "{}".format(width_center[i]),
                                                                        "-setnumber", "width_Core_Leg", "{}".format(width[j]),
                                                                        "-setnumber", "thickness_Core", "{}".format(t[k])])
                temp_res = np.zeros([2,1])
                with open('Magnetodynamics\\Three_Phases\\ShellB\\UI3_shell.txt', newline='') as csvfile:
