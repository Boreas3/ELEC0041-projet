...
class XLSX_WRITE:
    def __init__(self, l, width_center, width, t):
        self.writer = pd.ExcelWriter("Magnetodynamics\\Three_Phases\\ShellA\\geotypeA2_{}.xlsx".format(l), engine='xlsxwriter')
...
if __name__ == "__main__":
    ...
    width_center = np.linspace(0.4, 0.6, N) # Largeur jambe centrale
    width = np.linspace(0.4, 0.6, N) # Largeur jambe externes 
        ...
        for i in range(0,N):
            for j in range(0,N):
                for k in range(0,N_t):
                    call(["gmsh", "-2", "Magnetodynamics/Three_Phases/ShellA/transfo_3p_shell2.geo",  "-v", "1", 
                    ...
                    call(["getdp", "Magnetodynamics/Three_Phases/ShellA/transfo_3p_shell2.pro",  "-v", "1", 
                                                                            "-setnumber", "width_Window", "{}".format(width_window[l]),
                                                                            "-setnumber", "central_width_Core_Leg", "{}".format(width_center[i]),
                                                                            "-setnumber", "width_Core_Leg", "{}".format(width[j]),
                                                                            "-setnumber", "thickness_Core", "{}".format(t[k]),
                                                                            "-solve", "Magnetodynamics2D_av", "-pos", "Map_a"])
                    temp_res = np.zeros([2,1])
                    with open('Magnetodynamics\\Three_Phases\\ShellA\\UI3_shell2.txt', newline='') as csvfile:
